...
**Keep sibling apps composable, not just decoupled.** A consequence worth designing for deliberately: if each sibling app is a thin router over the shared library (previous paragraph), then two or more of them can later be mounted into a single process — one merged nav, one running server — as a composition operation rather than a rewrite, *as long as* each app's router stays mountable by construction (no module-level global state, no hardcoded root path, no assumption that it's the only app in the process). This is the inverse of decoupling: an explicit "Integrate" capability that takes N independently-developed sibling apps and runs them tightly coupled, in-process, when a user wants one merged tool instead of several. It costs nothing to keep open — it's a property of writing clean routers, not extra plumbing — so it should be treated as a standing constraint on how sibling apps are built, not built as a feature itself. There is no second sibling app yet, so there is nothing to integrate; this is a principle to hold the line on when one is built, not a sprint to schedule.
