"""Unit tests for CorpusFilterSpec-based scope on get_pending_* functions."""
import pytest
...
def test_pending_describe_filter_by_type_image(scoped_corpus):
    conn, src1, src2, img1, img2, vid1, img3 = scoped_corpus
    rows = get_pending_describe_files(conn, scope=CorpusFilterSpec(file_type="image"))
    assert len(rows) == 3
...
def test_pending_describe_filter_by_type_video(scoped_corpus):
    conn, src1, src2, img1, img2, vid1, img3 = scoped_corpus
    rows = get_pending_describe_files(conn, scope=CorpusFilterSpec(file_type="video"))
    assert len(rows) == 1
...
def test_pending_describe_combined_filter(scoped_corpus):
    conn, src1, src2, img1, img2, vid1, img3 = scoped_corpus
    rows = get_pending_describe_files(conn, scope=CorpusFilterSpec(source_id=src1, file_type="image"))
    assert len(rows) == 2
    paths = {r["path"] for r in rows}
...
def test_pending_transcribe_filter_by_source(scoped_corpus):
    ...
    rows_src1 = get_pending_transcribe_files(conn, scope=CorpusFilterSpec(source_id=src1))
    assert len(rows_src1) == 1

    rows_src2 = get_pending_transcribe_files(conn, scope=CorpusFilterSpec(source_id=src2))
    assert len(rows_src2) == 0


def test_pending_transcribe_filter_by_type(scoped_corpus):
    conn, src1, src2, img1, img2, vid1, img3 = scoped_corpus
    rows = get_pending_transcribe_files(conn, scope=CorpusFilterSpec(file_type="video"))
    assert len(rows) == 1
...
def test_pending_quality_filter_by_source(tmp_path):
    ...
    rows_src1 = get_pending_quality_files(conn, scope=CorpusFilterSpec(source_id=src1))
    assert len(rows_src1) == 1
...
def test_pending_quality_filter_by_type(tmp_path):
    ...
    conn.commit()
    rows_img = get_pending_quality_files(conn, scope=CorpusFilterSpec(file_type="images"))
    assert len(rows_img) == 1
    rows_vid = get_pending_quality_files(conn, scope=CorpusFilterSpec(file_type="video"))
...
def test_pending_aesthetic_filter_by_source(tmp_path):
    ...
    assert len(rows_all) == 2
    rows_src1 = get_pending_aesthetic_files(conn, "nima_mobilenet", scope=CorpusFilterSpec(source_id=src1))
    assert len(rows_src1) == 1
...
def test_pending_summarize_filter_by_source(scoped_corpus):
    ...
    rows_src1 = get_pending_summarize_files(conn, scope=CorpusFilterSpec(source_id=src1))
    assert len(rows_src1) == 1  # only img1

    rows_src2 = get_pending_summarize_files(conn, scope=CorpusFilterSpec(source_id=src2))
...
def test_pending_retag_filter_by_source(scoped_corpus):
    ...
    rows_src1 = get_pending_retag_files(conn, scope=CorpusFilterSpec(source_id=src1))
    assert len(rows_src1) == 3

    rows_src2 = get_pending_retag_files(conn, scope=CorpusFilterSpec(source_id=src2))
