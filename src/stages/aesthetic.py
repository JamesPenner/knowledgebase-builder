...
def run_aesthetic(corpus_path, kb_path, config, progress, cancel, *, source_id=None, file_type=None) -> dict:
    ...
    if config.aesthetic_nima:
        pending = get_pending_aesthetic_files(conn, "nima_mobilenet", source_id=source_id, file_type=file_type)
        total = len(pending)
    ...
    if config.aesthetic_clip:
        pending = get_pending_aesthetic_files(conn, "clip_vit_b32", source_id=source_id, file_type=file_type)
        total = len(pending)
